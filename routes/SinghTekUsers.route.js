...
SinghTekRoute.post("/merchant/register", auth, async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({ message: "Merchant registered successfully with basic details" });
  } catch (error) {
    console.log(error);
    res
      .status(500)
      .json({ error: "An error occurred during merchant registration" });
  }
});
...
SinghTekRoute.patch(
  ...
  async (req, res) => {
    try {
      ...
      const { address, pincode, city, state, country } = businessAddress;
...
SinghTekRoute.get("/dashboard/data", async (req, res) => {
  try {
    ...
    const uniqueMerchants = await Merchant.distinct("_id", {
      singhtek_id: req.body.userId,
    }).count();
  ...
  try {
    ...
    for (const data of updatedData) {
      ...
      if (data.bank_status === "Reject") {
        try {
          const wdata = await Withdrawal.findOne({
            withdrawal_id: data.remarks_1,
          }).exec();
          if (wdata) {
            ...
            const newPayment = new Payment({
              user_name: "", // Placeholder for merchant username
              amount: wdata.amount,
              type: "credit",
              timestamp: new Date(), // Current date and time
              account_number: "", // Placeholder for merchant account number
              merchantID: wdata.merchantID,
              remark: "Bank Request Rejected Amount",
            });

            const merchant = await Merchant.findById(wdata.merchantID).exec();
            // console.log(merchant);
            if (merchant) {
              newPayment.user_name = merchant.user_name; // Assign merchant's username
              newPayment.account_number = merchant.bank_account_number; // Assign merchant's account number
            }
